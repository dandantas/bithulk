...
export class BitbucketService {
  ...
  async getCommitDiff(options: BitbucketDiffOptions): Promise<string> {
    try {
      ...
      logger.error(`Error fetching commit diff: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  parseDiff(rawDiff: string): ParsedDiff[] {
    ...
    while ((match = fileRegex.exec(rawDiff)) !== null) {
      ...
      while ((hunkMatch = hunkRegex.exec(fileDiffContent)) !== null) {
        ...
        const oldLines = hunkMatch[2] ? parseInt(hunkMatch[2], 10) : 0;
        const newStart = parseInt(hunkMatch[3], 10);
        const newLines = hunkMatch[4] ? parseInt(hunkMatch[4], 10) : 0;
